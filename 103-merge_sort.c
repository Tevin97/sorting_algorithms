...
/**
* TMerge - sorts and merges the sub arrays
...
/**
* TSplitMerge - recursively splits the array, merge and sort
* @start: starting index (inclusive)
* @end: end index (exclusive)
* @array: the array to sort
* @clone: a clone of the array
*/
void TSplitMerge(size_t start, size_t end, int *array, int *clone)
{
	size_t middle;
	...
	if (end - start < 2)
		return;
	middle = (start + end) / 2;
	TSplitMerge(start, middle, array, clone);
	TSplitMerge(middle, end, array, clone);
	TMerge(start, middle, end, array, clone);
...
void merge_sort(int *array, size_t size)
{
	...
	for (i = 0; i < size; i++)
		clone[i] = array[i];
	TSplitMerge(0, size, array, clone);
	free(clone);
}
